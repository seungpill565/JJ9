...
<c:forEach items="${purchase }" var="purchase">
		...
		<div id="reivew">
			...
			<div>아이디 : ${reivew.member_id }</div>
				<div id="star${reivew.review_grade }">
					<img alt="" src="/jj9/resources/images/star/star${reivew.review_grade }.PNG">
	...
	<div id ="seller-info">
		<h3>판매자 정보</h3>
		<div>${purchase.member_nickName }</div>
		<button id="window-test" >연습</button>
			<div id="pop-url" style="display: none;"><c:url value="/message/${purchase.talent_id}"/></div>
		<form action="<c:url value="/message/${purchase.talent_id }"/>" method="GET">
			<input id="order-button" type="submit" value="1:1쪽지 보내기">
		</form>
		<h3>소개</h3>
		<div>${purchase. member_comments }</div>
	</div>
